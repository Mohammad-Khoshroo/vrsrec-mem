"""Tests for vrsrec_mem.formatter."""
...
from vrsrec_mem.formatter import (
    WIDTH_MAP,
    detect_gaps,
    format_output,
...
class TestFormatOutput(unittest.TestCase):
    ...
    def test_byte_mem_no_address_contiguous(self):
        ...
        self.assertEqual(
            out.splitlines(), ["11", "22", "33", "44", "55", "66", "77", "88"]
        )
    ...
    def test_byte_txt_with_address(self):
        out = format_output(
            make_memory(), out_format="txt", with_address=True, err=io.StringIO()
        )
    ...
    def test_byte_csv_with_address(self):
        out = format_output(
            make_memory(), out_format="csv", with_address=True, err=io.StringIO()
        )
    ...
    def test_byte_csv_no_address(self):
        out = format_output(
            make_memory(), out_format="csv", with_address=False, err=io.StringIO()
        )
        lines = out.splitlines()
    ...
    def test_word_mem_little_endian(self):
        out = format_output(
            make_memory(), data_width="word", endian="little", err=io.StringIO()
        )
    ...
    def test_word_mem_big_endian(self):
        out = format_output(
            make_memory(), data_width="word", endian="big", err=io.StringIO()
        )
    ...
    def test_halfword_little(self):
        out = format_output(
            make_memory(), data_width="halfword", endian="little", err=io.StringIO()
        )
        self.assertEqual(out.splitlines(), ["2211", "4433", "6655", "8877"])

    def test_halfword_big(self):
        out = format_output(
            make_memory(), data_width="halfword", endian="big", err=io.StringIO()
        )
